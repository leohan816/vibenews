...
카테고리/예약 설정 자체는 기존 mock/local skeleton이다. 다만 play 진입은 18번 MVP의 실제
user-global automatic state를 사용한다. Briefing은 Add로 교체된 하단 탭이 아니라 Listen/Add에서 여는
push flow다.
...
- 기본 카테고리 집합 확정(AI/건강/투자/피부관리/뷰티/비즈니스…).
...
다중 사용자 선택/추천은 future다. fixed user `leo`의 이 MVP에서는 화면별 filtered plan을 만들지 않고
한 전역 automatic snapshot을 사용한다. 실제 server-ready ContentItem만 들어가며 mock session을
acceptance에 사용하지 않는다.
...
| `topicCategory` | 큰 주제가 무엇인가 | News · AI · Health · Finance · Skin Care · Beauty · Business · Developer · Science · Lifestyle · Internal                                        |
...
tags:             [React Native, Performance, Monitoring]
entities:         [Expo, React Native, Observe]
```
...
- Entity는 개별 고유명(예: `Expo`, `React Native`, `expo/expo`). **반복적으로 주제화될 때만**
  tag로 승격한다(예: `Expo`가 계속 흐름의 중심이면 `Expo` tag 검토).
...
| 4. 제외               | 전역 `completed` · `skipped` · **중복** 후보 제거 |
...
장기 section-level 개인화는 future다. 이 MVP의 YouTube ContentItem은 실제 public caption,
DeepSeek Builder, 별도 DeepSeek Verifier, Fish Audio pipeline을 통과하며 exact contract는 18번 문서다.

## Source Pool·Subcategory와 브리핑 조립

브리핑은 아무 콘텐츠나 모아 만드는 게 아니라, **Source Pool에서 온 후보 중 점수 높은 것만**
ContentItem으로 승격해 조립한다. 여기서는 이 문서의 카테고리/오늘의 흐름 UX가 어떤 계층과
어떤 후보 선별을 근거로 하는지를 설계 레벨로 정리한다(타입 정본:
[10_DataModel](10_DataModel_데이터구조.md), 원천·큐레이션 상세:
[15_Source_Pool_and_Editorial_Curation](15_Source_Pool_and_Editorial_Curation.md)).

### 1) 계층 확정: Category → Subcategory → TopicCluster → Tag → Entity

| 계층            | 정의                                       | 규모        | 용도                          |
| --------------- | ------------------------------------------ | ----------- | ----------------------------- |
| **Category**    | 큰 방(고정 대분류)                         | 8-10개      | 상단 가로 탭                  |
| **Subcategory** | Category 안의 세부 분류(신규)              | Category당 여럿 | UI 필터 · 개인화 양쪽        |
| **TopicCluster**| "오늘의 흐름"(오늘 묶인 브리핑 단위)       | 매일 가변   | 하단 흐름 카드                |
| **Tag**         | 세부 주제 라벨                             | 콘텐츠당 3-8 | 카드/상세 보조 표기          |
| **Entity**      | 회사·제품·사람·도구·repo 고유명            | 무제한      | 상세, 검색·연결 근거          |

- Subcategory 예: Health 안의 **수면 / 혈당 / 장건강 / 보충제 / 운동회복**.
- Subcategory는 **UI 필터**(Health 안에서 수면만 보기)와 **개인화**(수면 관심 사용자에게 가중치)에
  **모두** 사용한다. 어느 한쪽 전용이 아니다.

### 2) 조립은 승격된 ContentItem만 사용

- Personal Briefing Assembly는 **SourceCandidate에서 승격된 ContentItem만** 입력으로 쓴다.
- 후보(SourceCandidate)는 **editorial priority · hot topic · user interest · risk**로 점수화되고,
  **selected 후보만** fetch/analyze되어 ContentItem으로 승격된다(비용·품질 관리).
- 즉 브리핑 큐에 들어올 수 있는 것은 "점수 통과 → 승격"을 거친 콘텐츠뿐이며, 모든 후보를
  처리하지 않는다.

### 3) 오늘의 흐름(TopicCluster)은 하위 계층에서 도출

- "오늘의 흐름" 카드(TopicCluster)는 위에서 임의로 정하는 게 아니라 **Subcategory / Tag / Entity**에서
  도출한다(예: 수면 Subcategory + `#혈당`·`#단백질` tag + `GLP-1` entity가 묶여 "수면·대사 건강" 흐름).
- 같은 Global News Pool이라도 사용자별 Subcategory/Tag/Entity 관심에 따라 흐름 카드 구성은 달라진다.
- 원천 4유형·QualityPrediction·후보 점수화 상세:
  [15_Source_Pool_and_Editorial_Curation](15_Source_Pool_and_Editorial_Curation.md).

### 현재 상태

일반 Source Pool/Hot Topic/자동 taxonomy는 future다. 단 manual YouTube batch와 승인된 channel discovery는
18번 private MVP에서 실제 fetch/분석/TTS로 승격되며 다른 source나 mock으로 대체하지 않는다.

## 후보 승인 후 조립

Personal Briefing Assembly는 **승인·검수를 통과한(TTS-ready) ContentItem만** 사용한다. AI가 모든
콘텐츠를 자동으로 음성화하지 않는다.

- 파이프라인: manual CTA 승인 또는 channel ON standing approval → public caption → DeepSeek Builder →
  별도 DeepSeek Verifier → 9.0/10 이상이고 critical failure 없음만 Fish Audio.
- approval 전에는 caption/provider/TTS를 호출하지 않는다. Verifier는 총 **최대 2회**, 실패 시
  `human_review_required`이며 TTS/automatic queue 진입이 없다.
- 즉 브리핑 조립 큐에 들어오는 것은 "승인 → 검수 통과 → TTS-ready"를 모두 거친 ContentItem뿐이다.
- 상세 파이프라인: [16_Candidate_Review_and_TTS_Approval_Pipeline](16_Candidate_Review_and_TTS_Approval_Pipeline.md).

## YouTube MVP의 한 전역 briefing session

이 절은 위의 장기 개인화 구상 중 fixed user `leo` MVP 재생 동작을 구체화한다.

- Today, Listen global, Category, Tag, 오늘의 흐름은 모두 같은 server `global_playback_state`와 active
  session을 사용한다. Category/Tag는 entry context이지 queue filter가 아니다.
- 새 session은 active `in_progress`를 첫 번째로 두고, snapshot 시각까지 `audio_ready`인 `unheard`를
  `audioReadyAt ASC, contentItemId ASC`로 고정한다.
- session 시작 후 ready가 된 항목은 그 snapshot에 추가하지 않는다. cold start/새 session에서만 포함한다.
- `completed`, `skipped`, deleted는 모든 자동 surface에서 제외한다. manual replay는 상태나 automatic
  resume pointer를 바꾸지 않는다.
- device exit position, persistence cadence, conflict, A/B/C/D와 2:14 acceptance의 정본은
  [18번 §10](18_YouTube_Add_Global_Resume_MVP.md#10-사용자-전역-자동-재생과-resume)이다.
