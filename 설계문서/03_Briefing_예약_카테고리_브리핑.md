...
**future / 설계 전용** — Source Pool 수집, HotTopic 감지, 후보 점수화·승격, Subcategory 자동 분류는
모두 mock. 이번 작업은 설계·문서만이며 실제 fetch/분석/TTS는 구현하지 않는다.
...
- 파이프라인: `SourceCandidate → CandidatePreview → MD/Leo(Admin) 승인 → 승인분만 비싼
  분석·스크립트·DeepSeek 검수 → 9/10점 이상만 TTS-ready`.
- approval 전에는 full fetch/TTS를 하지 않는다. 자동 수정은 **최대 2회**, 실패 시 human review.
