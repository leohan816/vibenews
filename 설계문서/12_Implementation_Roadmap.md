...
5. repo/server 사실이 설계와 다르면 `DESIGN_DEFECT`로 멈춘다.
...
- raw migration 001로 User/Batch/Channel/Discovery/Source/Job/Attempt/TempArtifact/Taxonomy/Content/Audio/
  Playback/Session/DailyUsage/Lease/Audit tables를 만든다.
...
outcome-unknown/crash/partial/duplicate, day-boundary defer tests.

### Block 6 — Add tab과 기존 Briefing 보존

- 하단 Briefing tab을 Add로 교체하고 기존 Briefing UI는 root push route로 옮긴다.
- 최대 10 URL, item별 validation/progress/failure isolation, CTA explicit approval를 구현한다.
- 최대 5 channel, ON/OFF standing approval, delete/tombstone, poll/defer status를 구현한다.
...
AudioAsset ID, playback state/revision뿐이다.
...
| 3 | ephemeral caption chunks + evidence refs + metadata | DeepSeek Builder chunk/aggregate | strict `builder-chunk-output.v1` then `builder-output.v1` |
| 4 | independent rubric/evidence + Builder output | DeepSeek Verifier | strict `verifier-output.v1` |
| 5 | score/gate | server | PASS only at >=9.0 and no critical; max 2 |
| 6 | passed audio script | Fish TtsProvider | verified temp mp3 |
| 7 | valid TTS artifact + DB/files | worker receipts/staging/finalize | provider success receipt/count once, then exactly one private AudioAsset `ready` |
| 8 | ready item | automatic session service | active first + immutable unheard snapshot |
| 9 | authorized AudioAsset | Expo global player | persisted playback/mutation/event |
