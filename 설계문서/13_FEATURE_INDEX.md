...
| Manual YouTube batch               | 최대 10 URL, CTA explicit approval + D-009-A public low-risk technology scope attestation, item별 progress/failure isolation                                  | Add                        | ManualBatch, ManualBatchItem, ProviderScopeApproval, ProcessingJob | design-ready | API/UI 구현                           | 15, 16, 18       |
| Approved YouTube channel           | 최대 5, ON fresh scope approval/OFF revoke, hourly discovery, poll당 unseen 최대 3                                                                            | Add                        | Channel, ChannelDiscovery, ProviderScopeApproval                  | design-ready | poller/UI 구현                        | 15, 16, 18       |
...
| Pre-provider scope/payload guard   | current public YouTube low-risk technology approval + exact role allowlist; expanded/ambiguous scope는 network 전에 Leo/GPT escalation                     | Add/server worker          | ProviderScopeApproval, ProviderPayloadAudit                       | design-ready | local guard/audit tests               | 10, 11, 12, 15, 16, 18 |
| Provider policy assurance record   | DeepSeek/Fish official URL/date/review/API surface/statements + private runtime binding + exact LIMITED/NOT_VERIFIED labels; uncertainty alone은 current slice 비차단 | server worker/acceptance   | ProviderPolicySnapshot, ProviderRuntimeBinding                    | design-ready | snapshot/binding/label tests          | 10, 11, 12, 16, 18 |
| Fish Audio TTS generation          | final approved SpokenAudioScript + configured reference/minimum params only → exactly one private AudioAsset                                                  | server worker              | AudioScript, ProviderPayloadAudit, AudioAsset, DailyTtsUsage      | design-ready | guarded atomic finalize               | 02, 12, 16, 18   |
...
| User Interest Profile              | 사용자 취향 프로필(관심/부정/톤/길이); D-009-A current DeepSeek/Fish payload에는 미사용                                                                       | Settings                   | UserInterestProfile                                               | future      | 프로필 저장                           | 09, 10, 18       |
...
- D-009-A provider policy assurance record와 exact five labels
- current-scope pre-provider guard, DeepSeek/Fish payload minimization, expanded-scope Leo/GPT stop
