...
상태: `mock` 화면만/가짜데이터 · `partial` 일부동작 · `working` 실동작 · `future` 미착수.

| 기능 | 설명 | 화면 위치 | 데이터 모델 | 상태 | 다음 구현 단계 | 관련 문서 |
| --- | --- | --- | --- | --- | --- | --- |
| 중앙 재생 버튼 | 앱 첫 화면의 큰 재생 버튼 + 개인화 문구 | Listen (`(tabs)/index`) | BriefingSession, Category | mock | 실제 오디오 시작 연결 | 02 |
...
| Daily Recap | 오늘 들은 내용 요약(음성+텍스트+카드) | Recap, DailyRecapDetail | DailyRecap, RecapCard | mock | 청취로그 기반 AI 요약 | 04 |
...
## 반드시 포함(확인)
중앙 재생 버튼 · 오디오 플레이어 · 챕터 이동 · 이전/다음/다시 듣기 · 저장 · 더 알아보기 · Daily Recap · 저장 카드 · 카테고리 브리핑 · 예약 브리핑 · 음성 명령 · 개인화 상품 추천 · Foundation 뉴스 지식 후보 · Event Log · GitHub 보안 원칙 — **모두 위 표에 존재.**
