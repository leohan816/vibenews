...
확정된 D-001~D-009-A를 다시 결정하지 않는다. D-009-A에 따라 이 설계가 허용하는 live provider 범위는
Leo가 승인한 **공개 YouTube의 저위험 기술 콘텐츠**인 private/internal MVP뿐이다. Provider 보존·학습·삭제
control의 불확실성은 이 현재 범위를 막지 않지만, local scope/payload control 실패 또는 아래에 열거한 확대
범위는 provider call 전에 fail closed한다. 특히 이전의 five-second exclusion rule은 삭제되었다.
...
- private/user-uploaded 문서, internal company data, personal conversation/memory 또는 third-party customer
  content를 provider로 보내는 것
- personal-data health/finance/legal/election use case, children/biometric data, confidential/regulated information,
  multi-user production, public commercial launch

## 2. 확인된 현재 상태

- Revision input head `5c93fe01fefa1927244588e114ffa1a0f565c6ff`에서 previous design content
  `f8a0dc01b7eede5ac9cfd0fc39157cb08cd7f984` 뒤 product/runtime blob은 바뀌지 않았고 D-009-A만 이 version 2의
  confirmed authority로 추가되었다.
...
/var/lib/vibenews-dev/private/provider-audit-hmac-v1.key
```
...
`private/`도 service user `0700`, audit key는 `0600`이다. Initial provisioning은 binding row가 없을 때만
32 random bytes를 `O_CREAT|O_EXCL`로 만들고 content를 절대 출력하지 않는다. Key는 Git, SQLite, log, result,
ordinary DB/audio backup에 없고 safe constant ID `provider-audit-hmac-v1`만 row/event에 남는다. Binding row가
있는데 key가 없거나 mode/owner가 틀리면 새 key로 덮지 않고 local runtime-binding readiness를 fail closed한다.
...
- CTA `분석·음성 생성`을 누르는 것이 해당 batch의 명시적 승인과 D-009-A scope attestation이다. 제출
  copy는 `공개 YouTube의 저위험 기술 콘텐츠만 분석합니다. private 문서·개인 대화·민감정보는 넣지
  마세요. 공개 자막을 분석하고 개인용 음성을 만들며 원본 영상·음성은 내려받지 않아요.`다. API는
  `scopeAttestationVersion='d009a.public-youtube-tech.v1'`과 `scopeAttested=true`가 아니면 batch를 만들지
  않는다.
...
- toggle ON copy: `이 공개 기술 채널의 새 공개 자막 영상을 매시간 확인해 자동 분석·음성 생성을
  승인합니다. private·personal·민감 콘텐츠가 되면 먼저 중지하고 범위 승인을 다시 받아야 합니다.`
  ON create/patch도 같은 D-009-A attestation version과 `scopeAttested=true`를 요구한다.
...
### 6.5 D-009-A current-scope gate

Live provider 처리는 `SourceScope='public_low_risk_youtube_technology'` 하나만 허용한다. Manual batch는
제출 때, channel은 최초 ON 및 다시 ON할 때 Leo의 versioned scope attestation을 DB에 저장한다. 이 attestation은
provider policy 승인이나 production privacy 승인이 아니라 **선택한 source가 현재 범위에 맞다는 사용자 승인**이다.
Channel approval version이 바뀌면 이전 discovery의 scope approval도 무효화한다.

`ProviderPayloadGuard`는 provider가 아니라 local deterministic code다. Caption은 public acquisition 후
ephemeral local file에만 있고, 다음 검사를 모두 통과하기 전에는 DeepSeek/Fish network request를 만들지 않는다.

1. source가 exact public YouTube canonical provenance를 갖고 active scope approval/version에 묶여 있다.
2. metadata와 ephemeral caption을 local denied-category/personal-data/private-marker scanner로 검사한다.
3. 요청 객체를 provider/stage별 strict allowlist schema로 새로 구성하고 unknown key를 거부한다.
4. semantic payload의 recursive field-name set, byte count, payload SHA-256, guard version만 audit하고 값과 body는
   기록하지 않는다.
5. adapter가 configured endpoint binding과 provider role을 확인한 뒤에만 authorization header를 붙인다.

High-confidence deny signal뿐 아니라 범위를 확정할 수 없는 경우도 `scope_review_required`로 멈춘다. Builder
결과가 `health | finance | skin_care | beauty | lifestyle | internal`이거나, `news | business`이면서 local
technology scope evidence와 충돌하는 등 attested public technology scope와 모순되면 추가 provider/TTS를 막는
defense-in-depth check를 한다. Category/model output은 scope를 새로 허용하는 근거가 아니다. Builder/Verifier를
scope classifier 또는 gate authority로 사용하지 않는다.

```ts
type SourceScope = 'public_low_risk_youtube_technology';
type ExpandedScopeReason =
  ...
  | 'scope_ambiguous';
type ProviderPayloadGuardOutcome =
  | 'allowed'
...
어느 `ExpandedScopeReason`이든 job은 `human_review_required`와
`SCOPE_ESCALATION_REQUIRED`로 끝나며 Leo/GPT의 새 explicit decision 전에는 retry·toggle·operator override로
진행할 수 없다. 반대로 public provider policy lookup 실패나 provider-side control 미검증만으로는 이 현재
범위를 막지 않는다. 그것은 §7.5의 limited/unverified evidence로 기록한다.
Scope finding은 per-job/payload-audit state다. 한 manual item의 finding이 shared batch approval이나 compliant
sibling을 revoke/cancel하지 않고, 한 channel discovery finding도 다른 discovery row를 삭제하지 않는다.
Expanded scope가 별도 설계 대상으로 승인되더라도 provider-side control 미검증은 그 release의 blocker이며,
필요한 protection을 만족하는 provider 또는 self-hosted model을 Leo/GPT가 따로 승인해야 한다. 이는 current
MVP에 alternate/self-hosted provider를 추가하는 권한이 아니다.
...
- Builder chunk context는 processing에 필요한 public source metadata와 **한 ephemeral public caption
  chunk/evidence refs**다. Aggregate context는 같은 public metadata와 strict-parsed generated chunk outputs만
  쓰며 raw caption 전체를 다시 넣지 않는다. User preference, history, notes, conversation 또는 다른 private
  project context는 어느 DeepSeek request에도 없다.
...
`ProviderPayloadGuard`의 DeepSeek semantic allowlist는 다음처럼 닫혀 있다. Public source metadata는
`videoId`, `channelId`, canonical public YouTube URL, public title, published time, duration, caption language/kind,
prompt/schema version만 뜻한다.

| Role | 허용 payload | 금지 payload |
| --- | --- | --- |
| Builder chunk | processing에 필요한 public caption chunk text, evidence refs, public source metadata | 다른 chunk/문서, user preference/history, notes/conversation, credential, account/payment/personal data |
| Builder aggregate/revision | strict BuilderChunkOutput들, public source metadata, revision이면 allowlisted finding code/evidence refs와 prior strict candidate | raw caption 전체, verifier 자유문, hidden context, private project docs 또는 user data |
| Verifier | strict candidate, public source metadata, public caption에서 resolve한 ephemeral evidence pack, attempt number | user preference/history, Builder hidden context, unrelated transcript/document, credential 또는 private data |

Guard는 prompt envelope와 위 semantic object를 서로 분리해 parse하고 recursive unknown key를 거부한다.
HTTP serialization 직전 한 번 더 exact field-name snapshot을 검사하므로 prompt string 안에 JSON으로 감춘
비허용 context를 넣는 것도 금지한다. 허용 payload 외에는 adapter에 전달할 수 있는 generic map/extra-body
escape hatch가 없다.

DeepSeek 전 role 공통 금지는 private project document, personal note/conversation, credential/secret, user
account/payment data, health record, children's/biometric data, confidential business information, non-public
copyrighted material이다. Public YouTube transcript라 해도 current low-risk technology scope와 processing에
필요한 chunk/evidence가 아니면 전송하지 않는다.
...
`TtsInput`은 local orchestration DTO이고 `contentItemId`/`scriptHash`는 outbound body가 아니다. Verifier
PASS, server-computed score `>=9.0`, no critical failure를 만족한 **final approved SpokenAudioScript**만 Fish
semantic payload로 재구성한다. Fish allowlist는 그 script text, configured voice reference identifier, 그리고
format/language/speed처럼 adapter contract가 요구하는 minimum synthesis parameters뿐이다. Raw transcript,
VideoContentMap, AnalyticSummary, verifier evidence, playback/user history, private user data, app identifiers,
credentials/secrets는 넣지 않는다. Model selector는 runtime adapter protocol binding이고 extra product context가
아니다. Semantic guard가 통과한 뒤 adapter가 configured protocol selector와 authorization header를 붙이며,
guard 앞의 object에는 `FISH_API_KEY`가 존재할 수 없다.
...
### 7.5 D-009-A provider policy evidence와 assurance

Provider public policy는 runtime secret이 아닌 versioned evidence다. Worker 구현은 아래 public facts를 seed한
snapshot과 review date를 보존하고, acceptance 때 official URL의 lookup status/document-set hash를 갱신한다. Public
lookup이 unavailable이거나 문서가 바뀌었어도 `LIMITED_AND_UNVERIFIED`로 진실하게 기록하며, 그것만으로
현재 public low-risk technology slice를 막지 않는다. Designer는 실제 configured account/model/tier/control을
읽거나 live call로 검증하지 않았다.

Migration에는 public seed constants/schema만 둔다. Exact-label `ProviderPolicySnapshot`은 runtime-local scope/
payload/log/ephemeral-deletion controls가 acceptance preflight에서 통과한 뒤에만 생성하고 attempt에 연결한다.
따라서 test 전부터 `LOCAL_DATA_CONTROLS: VERIFIED`라고 주장하지 않는다. 실패하면 provider call은 local-control
failure로 block하고 exact acceptance record를 만들지 않는다.

| Provider | official policy / API evidence | policy date; reviewed | public statement summary | independent limitation |
| --- | --- | --- | --- | --- |
| DeepSeek | [Privacy Policy](https://cdn.deepseek.com/policies/en-US/deepseek-privacy-policy.html), [Model Mechanism](https://cdn.deepseek.com/policies/en-US/model-algorithm-disclosure.html), [API docs](https://api-docs.deepseek.com/) | Privacy Policy last update 2026-02-10; reviewed 2026-07-11 | inputs may be collected and used to improve/train technology; opt-out/deletion rights are stated; retention is purpose/business/legal based; downstream app end-user processing is outside that policy | configured account/model opt-out, no-training effect, retention window, and provider-side deletion were not independently verified |
| Fish Audio | [Privacy Policy](https://fish.audio/privacy/), [TTS API](https://docs.fish.audio/api-reference/endpoint/openapi-v1/text-to-speech), [official free-tier statement](https://fish.audio/pt/blog/s2-1-pro-free-api/?articleLocale=en) | Privacy Policy effective 2024-08-28; official free-tier statement published 2026-06-23 and modified 2026-06-29; reviewed 2026-07-11 | User Content collection and research/analytics/product-development use are stated; retention follows system need; deletion rights are stated; the free-tier statement says free-tier requests may improve model quality | configured tier/model, no-training effect, retention window, and provider-side deletion were not independently verified |

Public API surface IDs는 DeepSeek `POST /chat/completions`, Fish `POST /v1/tts`다. 실제 configured endpoint
origin/model/reference가 이 surface를 사용했다는 증거는 §8.2의 role-specific HMAC binding으로 남기며 원값을
DB/report에 복사하지 않는다. DeepSeek public statement codes는 `INPUT_COLLECTED`,
`SERVICE_IMPROVEMENT_OR_TRAINING_STATED`, `OPT_OUT_RIGHT_STATED`, `DELETION_RIGHT_STATED`,
`PURPOSE_BUSINESS_LEGAL_RETENTION_STATED`, `DOWNSTREAM_APP_PROCESSING_NOT_COVERED`다. Fish codes는
`USER_CONTENT_COLLECTED`, `RESEARCH_ANALYTICS_PRODUCT_DEVELOPMENT_STATED`, `SYSTEM_NEED_RETENTION_STATED`,
`DELETION_RIGHT_STATED`, `FREE_TIER_MODEL_IMPROVEMENT_STATED`다. Both-provider unverified codes는
`CONFIGURED_ACCOUNT_NO_TRAINING_EFFECT`, `CONFIGURED_ACCOUNT_OPT_OUT_EFFECT`, `PROVIDER_RETENTION_WINDOW`,
`PROVIDER_SIDE_DELETION`; Fish에는 `CONFIGURED_TIER`도 추가한다. Verified local codes는 `VERSIONED_SCOPE_ATTESTATION`,
`PRE_PROVIDER_PAYLOAD_ALLOWLIST`, `NO_RAW_PROVIDER_BODY_LOG`, `EPHEMERAL_CAPTION_LOCAL_DELETION`뿐이다.

```ts
type ProviderPolicyAssurance = 'LIMITED_AND_UNVERIFIED';
type LocalDataControls = 'VERIFIED';
type ProviderSideDeletion = 'NOT_VERIFIED';
type ProviderSideNoTraining = 'NOT_VERIFIED';
type ProductionPrivacyApproval = 'NOT_GRANTED';
type ProviderPolicyLookupStatus = 'retrieved' | 'unavailable' | 'changed_since_review';
type ProviderPublicStatementCode =
  | 'INPUT_COLLECTED'
...
Strict constructor는 위 표의 provider별 exact statement/unverified code set, official host/URL set, primary policy
date와 five labels를 검사하고 unknown/missing code를 거부한다. `CONFIGURED_TIER`는 Fish row에만 허용한다.

Acceptance/result에는 다음 값을 **문자 그대로** 낸다.
...
`LOCAL_DATA_CONTROLS: VERIFIED`는 scope attestation/guard, strict outbound allowlist, no-body logging,
ephemeral local caption cleanup test가 통과했다는 뜻뿐이다. 다음 주장은 금지한다: local deletion이 provider
copy를 삭제한다; inputs are never retained; inputs are never used for training; provider-side deletion is verified;
private safeguards equal production compliance. Expanded scope는 §6.5의 Leo/GPT escalation을 통과하기 전까지
항상 block한다.
...
ProviderPolicyLookupStatus = retrieved | unavailable | changed_since_review
ProviderPayloadGuardOutcome = allowed | scope_review_required | payload_rejected |
                              runtime_binding_rejected
ScopeApprovalStatus = active | revoked | superseded
...
| `provider_scope_approvals` | `id`, user, origin kind plus nullable manual-batch/channel FKs with exactly-one CHECK, exact `source_scope='public_low_risk_youtube_technology'`, attestation version, status, approval version, approved/revoked timestamps; no source text | partial unique active `(manual_batch_id,approval_version)` and `(channel_id,approval_version)`; `(status,approved_at)` |
...
| `processing_jobs` | `id`, user/source/scope-approval FKs, origin kind plus nullable manual-item/channel-discovery FKs with exactly-one CHECK, approval version, state/stage, `eligible_at`, defer reason, verifier attempts `0..2`, lease owner/expiry/heartbeat, idempotency key, safe error, timestamps | unique idempotency and each non-null origin FK; `(state,eligible_at,created_at)`; `(lease_expires_at)` |
| `provider_policy_snapshots` | `id`, provider, official policy/API URLs, public API surface ID, policy effective/update date, reviewed time, nullable public document-set hash, lookup status, statement/control-code JSON, exact five assurance labels; no account/config value | unique `(provider,reviewed_at)`; `(provider,lookup_status,reviewed_at DESC)` |
| `provider_runtime_bindings` | `id`, provider role, public API surface ID, safe audit-key ID, endpoint-origin HMAC, model-selector HMAC, nullable reasoning/reference-selector HMAC, config-version hash, credential-presence boolean, verified time; no raw endpoint/model/reference/credential | unique `(provider_role,config_version_hash)`; `(verified_at)` |
| `provider_attempts` | `id`, job/macro stage, exact substage, `ordinal NOT NULL` (`builder_chunk` 1..20, all others 0), logical submission `1..2`, prompt/schema/config version hashes, request/output hashes, scope approval FK, nullable policy snapshot/runtime binding FKs that are mandatory for DeepSeek/Fish and null for local caption adapter, status/error, started/finished; no raw body | unique `(job_id,substage,ordinal,logical_attempt)`; indexes on three evidence FKs |
| `provider_payload_audits` | `id`, job FK, provider role, nullable provider-attempt FK (null when blocked before network), guard/scope-attestation versions, exact guard outcome, sorted recursive field-name JSON, nullable semantic payload bytes/hash, forbidden-field count, nullable expanded-scope reason, check ordinal/time; no values/body/text | unique `(job_id,provider_role,check_ordinal)`; unique non-null provider attempt; `(outcome,checked_at)` |
...
- 모든 `user_id`는 `users.id`를 참조한다. `manual_batch_items.batch_id -> manual_batches.id`,
  `channel_discoveries.channel_id -> channels.id`다. `provider_scope_approvals.manual_batch_id/channel_id`는
  exactly-one CHECK로 해당 origin을 참조한다. `processing_jobs.scope_approval_id`는 그 row를 참조하며 job origin,
  user, approval version 일치는 service invariant와 claim transaction check로 강제한다.
...
- `provider_attempts.job_id`, `provider_payload_audits.job_id`, `temporary_caption_artifacts.job_id`는
  `processing_jobs.id`를 참조한다. 모든 attempt는 job의 active `provider_scope_approvals.id`를 참조한다.
  DeepSeek/Fish attempt만 해당 provider의 `provider_policy_snapshots.id`와 exact role의
  `provider_runtime_bindings.id`를 반드시 참조하고 local caption attempt는 두 FK가 null이다.
  `provider_payload_audits.provider_attempt_id`는 network request가 실제로 생성된 allowed case에만 non-null이다.
- Guard는 먼저 audit row를 `provider_attempt_id=NULL`로 기록한다. Allowed case는 immediate transaction에서
  evidence FKs를 가진 attempt를 insert하고 audit FK를 연결한 뒤 commit하며, 그 뒤에만 serialize/header
  attachment/network call을 한다. Blocked case는 null을 유지한다. 따라서 call은 있으나 allowed audit이 없는
  crash window가 없다.
...
Runtime binding HMAC은 server-only local audit key로 endpoint origin/base path, configured model selector,
Verifier reasoning selector, Fish reference selector를 role-tagged canonical bytes에서 계산한다. Audit key와 원값은
Git/DB/log/result에 없고 operator-only verifier가 같은 runtime에서 recompute한다. HMAC·config version은 값의
공개나 provider-side control 검증을 뜻하지 않는다.

Exact encoding은 `HMAC-SHA-256(key, utf8('vibenews.provider-binding.v1\0' + providerRole + '\0' + fieldName +
'\0' + byteLength + '\0' + value))`의 unpadded base64url이다. `config_version_hash`는 public API surface ID,
role, 네 selector-HMAC field, adapter schema version의 canonical JSON SHA-256이다. Builder/Verifier/Fish 모두
endpoint+model HMAC 필수, Verifier만 reasoning HMAC 필수, Fish만 reference HMAC 필수이며 다른 nullable field는
null이어야 한다. Constructor/test가 이 role matrix를 강제한다.

D-009 CHECK matrix도 migration/service 양쪽에서 강제한다. `document_set_sha256`은 sorted
`[{url,effectiveOrUpdatedDate,bodySha256}]` canonical JSON의 SHA-256이다. Policy
`retrieved|changed_since_review`는 이 public document-set hash가 non-null이고 `unavailable`은 null이다. Payload `allowed`만 bytes/hash non-null,
`forbidden_field_count=0`, reason null이며 attempt와 연결된다. `scope_review_required`는 exact reason non-null,
bytes/hash null, field-name list empty, attempt null이다. `payload_rejected|runtime_binding_rejected`도 bytes/hash와
attempt가 null이고 safe field-name/count/code만 남긴다. 따라서 rejected private text의 hash조차 보존하지 않는다.
...
`DAILY_TTS_CAP_DEFERRED`, `HUMAN_REVIEW_REQUIRED`, `SCOPE_ESCALATION_REQUIRED`,
`PAYLOAD_GUARD_REJECTED`, `RUNTIME_BINDING_REJECTED`, `PROVIDER_UNAVAILABLE`, `STORAGE_UNAVAILABLE`,
...
interface CreateManualBatchRequest {
  urls: string[]; // min 1, max 10
  scopeAttested: true;
...
Channel DTO의 attestation field가 optional인 이유는 OFF path가 scope 승인을 요구하지 않기 때문이다.
`autoProcessingEnabled=true`인 create/patch는 Zod `superRefine`과 transaction에서 두 field의 exact true/version을
반드시 요구하고 새 active `provider_scope_approvals` row를 만든다. False는 active scope approval을 revoke한다.
...
| `POST /v1/manual-batches` | `{urls:string[1..10],scopeAttested:true,scopeAttestationVersion:'d009a.public-youtube-tech.v1'}`, Idempotency-Key | `202 {batchId,items[]}`; CTA approval/scope attestation과 item isolation |
...
| `POST /v1/channels` | `{url,autoProcessingEnabled,scopeAttested?,scopeAttestationVersion?}` + Idempotency-Key | ON이면 exact scope attestation; canonical channel; max-5 transaction |
| `PATCH /v1/channels/:id` | `{autoProcessingEnabled,scopeAttested?,scopeAttestationVersion?}` + Idempotency-Key | ON이면 새 scope approval/version; OFF이면 철회 |
...
- Provider를 쓰는 current scope는 approved public YouTube low-risk technology content뿐이다. §6.5의 private,
  personal, sensitive, multi-user/production/commercial/third-party/confidential 범위는 Leo/GPT 재결정 전 hard
  block이고, policy uncertainty만은 current slice의 hard block이 아니다.
...
- DeepSeek/Fish에는 §7.2/§7.4의 exact allowlist만 보낸다. Worker는 live call 전 active scope approval,
  deterministic payload guard, exact runtime binding, provider policy snapshot을 safe IDs/hashes/codes로 남긴다.
  Public policy lookup failure와 configured account의 no-training/deletion control 미검증은 exact D-009-A labels로
  보고하되 current slice를 막지 않는다. Local scope/payload/runtime binding guard failure는 provider call 전에
  hard block한다. VibeNews의 24시간 local deletion audit는 provider copy 삭제나 retention/training control의
  증거가 아니며 그런 주장을 만들지 않는다.
...
- D-009-A scope attestation/version, every `ExpandedScopeReason`, ambiguous scope, and retry/toggle bypass rejection
- DeepSeek/Fish recursive payload allowlists; user preference/history/private context never appears; Fish auth is added
  only after the semantic guard and internal content IDs never enter the wire body
- policy snapshot schema/public-statement codes, unavailable/changed lookup remains limited and nonblocking for this
  current slice, runtime binding mismatch blocks, and exact five assurance labels serialize literally
- assurance formatter never emits any prohibited provider-retention/training/deletion/production-compliance claim
...
- Missing/revoked/superseded scope approval and each expanded/ambiguous scope produce a payload audit with no network
  attempt; local mock captures prove exact stage field sets and absence of raw aggregate transcript, preferences,
  VideoContentMap/AnalyticSummary in Fish, secrets, and unrelated/private data
- Provider policy URL unavailable or document-set hash changed records `LIMITED_AND_UNVERIFIED` without blocking an
  otherwise-valid current source; bad local payload or configured runtime binding fails before network
...
10. active `public_low_risk_youtube_technology` scope approval and allowed payload-audit rows for each DeepSeek/Fish
    call, with field names/bytes/hashes only and no values/body
11. DeepSeek and Fish policy snapshots with official URLs, policy dates, review date, public statement codes, lookup
    status, plus private role-specific runtime-binding HMAC evidence for the configured API surface/model/reference
12. acceptance output contains exactly `PROVIDER_POLICY_ASSURANCE: LIMITED_AND_UNVERIFIED`,
    `LOCAL_DATA_CONTROLS: VERIFIED`, `PROVIDER_SIDE_DELETION: NOT_VERIFIED`,
    `PROVIDER_SIDE_NO_TRAINING: NOT_VERIFIED`, and `PRODUCTION_PRIVACY_APPROVAL: NOT_GRANTED`

Transient provider/YouTube failure is a truthful FAIL/BLOCKED runtime result, not permission to use sample audio or
claim PASS. Provider-policy/control uncertainty alone is not that failure for the bounded current scope; local scope,
payload, runtime binding, provider/API, or vertical-slice failures remain truthful blockers. No live provider call is
part of this Designer revision; these are Worker acceptance requirements.
...
- Provider: configured DeepSeek Builder/Verifier와 Fish credentials/model/reference availability, role-specific
  runtime-binding verification, official policy snapshot lookup. 실제 값은 출력 금지. Provider-side no-training,
  deletion, retention control은 검증 완료를 요구하지 않고 exact D-009-A NOT_VERIFIED labels로 남긴다.
...
실제 provider/API access가 없으면 live vertical-slice acceptance를 `NOT_RUN`/`BLOCKED`로 정확히 보고하고
sample로 대체하지 않는다. 단, official policy lookup 실패나 provider-side control 미검증만은 current-scope
product run의 blocker로 분류하지 않고 limited/unverified evidence로 기록한다.
...
1. D-001~D-009-A와 intake의 모든 요구가 문서/관련 product subjects에 보존됨
...
13. current public low-risk technology scope attestation, pre-provider local guard, role별 exact payload allowlist와
    expanded-scope Leo/GPT escalation이 provider call 전에 실행 가능함
14. 두 provider policy record, private runtime-binding evidence, exact five labels, prohibited-claim boundary가
    policy uncertainty를 blocker로 되돌리지 않으면서 independently checkable함
...
rg -n -i 'user.*preference|playback.*history|personal.*conversation|private.*document|raw.*transcript' server/src/providers server/src/services
rg -n 'DEEPSEEK_[A-Z_]+|FISH_[A-Z_]+' server src .env.example
...
- Provider의 가격/availability가 변할 수 있다. 명시된 cap은 비용 상한 행동을 고정하지만 금액을 보장하지
  않는다.
- DeepSeek/Fish의 configured account에 대한 retention, training, deletion control은 독립 검증되지 않았다.
  D-009-A는 이 residual risk를 current public low-risk technology private MVP에 한해 명시적으로 받아들인다;
  local deletion은 provider copy 삭제를 보장하지 않고 production privacy approval은 없다.
- Scope attestation/scanner는 오분류 위험이 있으므로 ambiguity를 fail closed한다. Private/personal/sensitive,
  customer/internal, multi-user/production/commercial 확대는 기술 구현 판단이 아니라 Leo/GPT 재결정 대상이다.
