...
## 목적
전 화면이 공유하는 TypeScript 타입을 한곳에 정의한다. 실제 구현은 `src/data/types.ts`.

## 현재 상태
**mock** — 타입은 확정, 데이터는 `src/data/mockData.ts`의 하드코딩 값.

## 타입 정의 (요약)
> 실제 필드는 `src/data/types.ts`와 동기화한다.

```ts
export type KnowledgeType = 'Fact' | 'Trend' | 'Insight' | 'Action' | 'Question';
export type FoundationStatus =
  | 'local_only' | 'candidate' | 'approved_for_foundation'
  | 'synced_to_foundation' | 'rejected';
