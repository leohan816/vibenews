...
IMPLEMENTATION_REVIEW_SUBJECT_PATHS: the implementation subject at the correction head (server/**, src/{api,audio,storage,components/add,app,lib,data} and deleted src/hooks/use-audio-player-controller.ts, ops/systemd/**, scripts/server-smoke.mjs, .env.example, app.json, tsconfig.json, package.json, package-lock.json, docs/{환경변수,실행방법,테스트방법,변경기록}.md + docs/구현로그/2026-07-11_youtube_add_global_resume_mvp.md) plus WORKER_RESULT.md; every file is stored as text at this head
